function setDB(options){
    return new Promise(function(resolve, reject){
        db.transaction(function (tx) {
            ...
            tx.executeSql('SELECT data FROM siks where '+where, [], function (tx, results) {
                try{
                    var data = results.rows.item(0).data;

                    tx.executeSql('UPDATE siks SET data="'+encodeURIComponent(JSON.stringify(options.data))+'" WHERE '+where);
                }catch(e){
                    var column = ['key', 'data'];
                    if(data){
                    ...
                    tx.executeSql('INSERT INTO siks ('+column.join(', ')+') VALUES ("'+value.join('", "')+'")');
                }
                return resolve(true);
            });
        });
...
function getDB(options){
    return new Promise(function(resolve, reject){
        db.transaction(function (tx) {
            ...
            var data = false;
            tx.executeSql('SELECT * FROM siks'+where, [], function (tx, results) {
                try{
                    if(options.key){
...
function rmDB(options){
    return new Promise(function(resolve, reject){
        db.transaction(function (tx) {
            ...
            tx.executeSql('DELETE FROM siks where '+where);
            return resolve(true);
        });
    })
    .catch(function(e){
